...
import { FileText, Filter, Phone, Mail, CheckCircle, FileText as DraftIcon, Clock, Layers, Copy, MessageCircle, Bell } from "lucide-react";
import MessagePreviewDialog, { type MessageRecipient } from './MessagePreviewDialog';
        ...
        <div className="flex flex-col lg:flex-row gap-4">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`
                    relative px-3 py-2 rounded-lg border-2 font-medium transition-all duration-200 
          ...
          <div className="flex flex-wrap gap-2 lg:ml-auto">
            <MessagePreviewDialog
              recipients={filteredInvoices.filter(inv => selectedInvoiceIds.includes(inv.id)).map(invoice => ({
                id: invoice.id,
                name: invoice.child?.fatherName || 'Parent',
                phone: invoice.child?.phone || '',
                email: invoice.child?.email || '',
                childName: invoice.child?.fullNameWithCaseId || '',
              ...
              trigger={
                ...
                >
                  ...
                  Bulk Fee Reminder ({selectedInvoiceIds.length})
                </button>
              }
            />
            ...
            <MessagePreviewDialog
              recipients={filteredInvoices.filter(inv => selectedInvoiceIds.includes(inv.id)).map(invoice => ({
                ...
                centre: invoice.centre,
                amount: invoice.total,
                paymentLink: `https://razorpay.me/@aaryavart/${invoice.total}`
              }))}
              ...
              trigger={
                ...
                >
                  <MessageCircle className="h-4 w-4" />
                  Bulk Notification ({selectedInvoiceIds.length})
                </button>
              }
            />
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-left py-3 px-4 font-semibold text-gray-700 min-w-[280px]">Actions</th>
              </tr>
            </thead>
            <tbody>
